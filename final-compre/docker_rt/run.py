...
with app.app_context():
    manage_table(doc=True) # drop all tables
                ...
                with frame_lock:  # Ensure thread-safe access
                    for cam_name, vs in list(vs_list.items()):  # Create a list to avoid runtime changes
                        ...
                        if not isinstance(frame, np.ndarray):
                            try:
                                print(f"frame type {type(frame)} while read")
                                frame = np.array(frame)
                            except Exception as e:
                                console_logger.error(f"Failed to convert frame to numpy array: {e}")
                                continue
                        ...
                        if frame is not None:
                            ...
                            if not isinstance(frame, np.ndarray):
                                try:
                                    print(f"frame type {type(frame)} before emit")
                                    frame = np.array(frame)
                                except Exception as e:
                                    console_logger.error(f"Failed to convert frame to numpy array: {e}")
                                    continue
